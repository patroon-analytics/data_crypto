...
class BinanceStreamProcessor:
    def __init__(self, ticker, dir_updates_root, db_ver, batch_seconds, batch_l2, timezone="US/Eastern"):
        ...
        self.batch_l2 = batch_l2
        self.timezone = pytz.timezone(timezone)
        self.messages = deque()
        self.lock = threading.Lock()
        self.last_l2_update = time.time()  # Initialize the last update timestamp
        logging.basicConfig(filename=f"logs/ws_bin_{self.ticker}.log", level=logging.DEBUG, format='%(asctime)s %(message)s')
        self.d_thresh = {
                        'BTCUSDT' : 500000,
                        'ETHUSDT' : 100000,
                        'SOLUSDT' : 10000, 
...
# ==================================================================================
    ...
    def process_batches(self):
        while True:
            for i in range(self.batch_seconds, 0, -1):
                print(f"Counter: {len(self.messages)} | Countdown: {i}", end='\r', flush=True)
                time.sleep(1)
    ...
    def process_hourly_batch(self):
        ...
        # Log the final length of messages
        logging.info(f"Final length of messages: {len(all_data)}")
        ...
        for data in all_data:
            try:
                ...
                if '@depth' in stream_type:
                    ...
                    xprint("***lodf_force_order_data***")                
                    xprint(lodf_force_order_data[-2:])          
            except Exception as e:
                logging.error(f"***process_hourly_batch()*** Error processing message: {data}")
...
# ----------------------------------------------------------------------------------
        ...
        if lodf_depth_data:
            ...
            logging.info(f"Depth DataFrame length: {len(df_depth)}")    
            print(f"Exporting: depth_{self.ticker.lower()}_{current_time}_{self.db_ver}.feather | Length: {len(df_depth)}")
        if lodf_trade_data:
            ...
            logging.info(f"Trade DataFrame length: {len(df_trade)}")
            print(f"Exporting: trade_{self.ticker.lower()}_{current_time}_{self.db_ver}.feather | Length: {len(df_trade)}")            
        if lodf_agg_trade_data:
            ...
            logging.info(f"AggTrade DataFrame length: {len(df_agg_trade)}")
            print(f"Exporting: aggTrade_{self.ticker.lower()}_{current_time}_{self.db_ver}.feather | Length: {len(df_agg_trade)}")            
        if lodf_kline_data:
            ...
            logging.info(f"Kline DataFrame length: {len(df_kline)}")
            print(f"ExExportingExportport: kline_{self.ticker.lower()}_{current_time}_{self.db_ver}.feather | Length: {len(df_kline)}")             
        if lodf_force_order_data:
            ...
            logging.info(f"ForceOrder DataFrame length: {len(df_force_order)}")
            print(f"Exporting: ForceOrder_{self.ticker.lower()}_{current_time}_{self.db_ver}.feather | Length: {len(df_force_order)}")
...
# ==================================================================================
    ...
    def fetch_and_process_order_book(self):
        ...
        if response.status_code == 200:
            ...
            logging.error(f"***fetch_and_process_order_book()*** Failed to fetch order book data: {response.status_code}")
    ...
    def create_df_kline(self, data_dict, ts_p_est):
        kline = data_dict['k']
        df = pd.DataFrame([{
            'Ticker': self.ticker,
            'Open': float(kline['o']),
        ...
        df['ts_open_est'] = self.convert_unix_to_est(df['OpenTime'][0])
        df['ts_close_est'] = self.convert_unix_to_est(df['CloseTime'][0]) 
        df = df.drop(['OpenTime', 'CloseTime'], axis=1)
        # logging.debug(f"Kline DataFrame: {df.head()}")
        xprint("***create_df_kline***")
        xprint(df.head())
        return df

    def create_df_liq(self, data_dict, ts_p_est):
        columns = ["Ticker", "Val_USD", "Price", "Quantity_Filled", "Quantity", "Liq_Type", "Side", "ts_e_est", "ts_p_est"]
        ...
        df['ts_e_est'] = self.convert_unix_to_est(df['EventTime_Unix'][0])
    ...
    def create_df_force_order(self, data_dict, ts_p_est):
        ...
        df['Val_USD'] = df['Price'] * df['Quantity_Filled']        
        df['ts_p_est'] = ts_p_est
        df['ts_e_est'] = data_dict['E']
        df['ts_e_est'] = self.convert_unix_to_est(df['ts_e_est'][0])        
...
if __name__ == "__main__":
    ...
    n_batch_l2 = 300  # Default value for batch_l2
    ...
    parser.add_argument("--a_batch_l2", type=int, default=n_batch_l2)
